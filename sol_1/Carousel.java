...
public class Carousel
{
    ...
    public synchronized Vial getC3Vial() throws InterruptedException
    {
        ...
        if(compartment[pickPosition - 1].isDefective() && !compartment[pickPosition - 1].inspected )
        {
    ...
    public synchronized void scanTheVial() throws InterruptedException
    {
        ...
        if(!compartment[2].isScanned())//i set an extra tag 'scanned', to show C3 is scanned or not yet
        {
            compartment[2].setIsScanned();
            if( compartment[2].defective)//not empty, is defective , not scanned yet
            {
                // that is let it requested to shuttle, i can choose use a extra tag but i
                //choose the combination ( defective ==true && tagged == false ) to represent
                // need but not yet inspected, ( defective ==true && tagged == true) to represent
                // need and finished inspection.
            }
        }
